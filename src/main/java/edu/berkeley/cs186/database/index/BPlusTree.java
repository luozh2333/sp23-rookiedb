...
public class BPlusTree {
    ...
    public Optional<RecordId> get(DataBox key) {
        ...
        return node.get(key).getKey(key);
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        // TODO(proj4_integration): Update the following line
        LockUtil.ensureSufficientLockHeld(lockContext, LockType.NL);
        /*it call different func depends on its root's type*/
        Optional<Pair<DataBox, Long>> thePairOfNewRoot = root.put(key, rid);
        if (thePairOfNewRoot.isPresent()) {
            List<DataBox> InnerNodeNewKeys = new ArrayList<>();
            List<Long> InnerNodeNewChildren = new ArrayList<>();
            InnerNodeNewKeys.add(thePairOfNewRoot.get().getFirst());
            InnerNodeNewChildren.add(root.getPage().getPageNum());
            InnerNodeNewChildren.add(thePairOfNewRoot.get().getSecond());
            InnerNode newRoot = new InnerNode(metadata, bufferManager, InnerNodeNewKeys, InnerNodeNewChildren, lockContext);
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
      ...
      if(!this.toSexp().equals("()")){
        ...
        node.bulkLoadHelp(data,fillFactor,ls);
        if(ls.size() == 1){
           return;
        } else {
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public BPlusTreeIterator() {
            leftLeafNode = root.getLeftmostLeaf();
            leafIndex = leftLeafNode.getRids().size();
        }

        public BPlusTreeIterator(DataBox key) {
            leftLeafNode = root.get(key);
            int index = 0;
            for (DataBox re : leftLeafNode.getKeys()) {
                if (key.compareTo(re) > 0) {
                    index++;
                }
            }
            leafIndex = leftLeafNode.getRids().size() - index;
        }
        ...
        public boolean hasNext() {
            ...
            if (leftLeafNode.getRightSibling().isPresent()) {
                leftLeafNode = leftLeafNode.getRightSibling().get();
                while (leftLeafNode.getRids().size() == 0 && leftLeafNode.getRightSibling().isPresent()) {
                    leftLeafNode = leftLeafNode.getRightSibling().get();
                }
                leafIndex = leftLeafNode.getRids().size();
            }
        ...
        public RecordId next() {
            if (hasNext()) {
                RecordId res = leftLeafNode.getRids().get(leftLeafNode.getRids().size() - leafIndex);
                leafIndex--;
                return res;
            }
