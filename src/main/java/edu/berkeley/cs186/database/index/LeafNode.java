...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        if (keys.contains(key)) {
            throw new BPlusTreeException("Insert same key");
        }
        int flag = 0;
        ...
        this.keys.add(flag, key);
        this.rids.add(flag, rid);
        if (thisOrder * 2 + 1 == keys.size()) {
            List<DataBox> newKeys = new ArrayList<>();
            List<RecordId> newRids = new ArrayList<>();
            for (int i = thisOrder; i < thisOrder * 2 + 1; i++) {
                newKeys.add(this.keys.get(thisOrder));
                newRids.add(this.rids.get(thisOrder));
                this.keys.remove(thisOrder);
                this.rids.remove(thisOrder);
            }
            Page newPage = bufferManager.fetchNewPage(treeContext, metadata.getPartNum());
            LeafNode newNode = new LeafNode(metadata, bufferManager, newPage, newKeys, newRids, this.rightSibling, this.treeContext);
            this.rightSibling = Optional.of(newPage.getPageNum());
            this.sync();
            return Optional.of(new Pair<DataBox, Long>(newKeys.get(0), newPage.getPageNum()));
        } else {
            this.sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
      ...
      /*  List<Optional<Pair<DataBox, Long>>> ls = new ArrayList<>();
        this.bulkLoadHelp(data, fillFactor, ls);
        return ls.size() > 1 ? ls.get(1) : Optional.empty();
*/

        int maxSize = Math.round(metadata.getOrder() * 2 * fillFactor);
        // full plus one
        int delta = maxSize - keys.size() + 1;
        ...
        // split a new LeafNode
        List<DataBox> keys = new ArrayList<>();
        List<RecordId> rids = new ArrayList<>();
        keys.add(this.keys.remove(maxSize));
        rids.add(this.rids.remove(maxSize));
        LeafNode newNode = new LeafNode(metadata, bufferManager, keys, rids, rightSibling, treeContext);
        rightSibling = Optional.of(newNode.getPage().getPageNum());
        newNode.sync();
        sync();
        return Optional.of(new Pair<>(keys.get(0), rightSibling.get()));
    }

    public void bulkLoadHelp(Iterator<Pair<DataBox, RecordId>> data,
                              float fillFactor, List<Optional<Pair<DataBox, Long>>> ls) {

        int fillSize = (int) Math.ceil(this.metadata.getOrder() * 2 * fillFactor);
        while (data.hasNext()) {
            if (this.keys.size() < fillSize) {
                Pair<DataBox,RecordId> nextPair = data.next();
                this.keys.add(nextPair.getFirst());
                this.rids.add(nextPair.getSecond());
            } else {
                List<DataBox> newKeys = new ArrayList<>();
                List<RecordId> newRids = new ArrayList<>();
                Page newPage = bufferManager.fetchNewPage(treeContext, metadata.getPartNum());
                LeafNode newNode = new LeafNode(metadata, bufferManager, newPage, newKeys, newRids, Optional.empty(), this.treeContext);
                this.rightSibling = Optional.of(newPage.getPageNum());
                ls.add(Optional.of(new Pair<DataBox, Long>(this.keys.get(0), this.getPage().getPageNum())));
                this.sync();
                newNode.bulkLoadHelp(data, fillFactor, ls);
                return;
            }
        }
        this.sync();
    ...
    public void remove(DataBox key) {
        if (this.keys.contains(key)) {
            int flag = this.keys.indexOf(key);
            this.keys.remove(flag);
            this.rids.remove(flag);
        }
        this.sync();
    }
    ...
    private void sync() {
        ...
        try {
            ...
            b.get(bytes);
            if (!Arrays.equals(bytes, newBytes)) {// if newBytes have data
