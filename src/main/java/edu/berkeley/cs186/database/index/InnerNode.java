...
class InnerNode extends BPlusNode {
    ...
    public LeafNode get(DataBox key) {
        int flag = numLessThanEqual(key, keys);
        Page p = bufferManager.fetchPage(treeContext, children.get(flag));
        try {
            Buffer buf = p.getBuffer();
            byte b = buf.get();
            if (b == 1) {
                return LeafNode.fromBytes(metadata, bufferManager, treeContext, children.get(flag));
            } else if (b == 0) {
                return InnerNode.fromBytes(metadata, bufferManager, treeContext, children.get(flag)).get(key);
            } else {
    ...
    public LeafNode getLeftmostLeaf() {
        ...
        try {
            ...
            if (b == 1) {
                ...
                return InnerNode.fromBytes(metadata, bufferManager, treeContext, children.get(flag)).getLeftmostLeaf();
            } else {
                String msg = String.format("Unexpected byte %b.", b);
                throw new IllegalArgumentException(msg);
            }
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        Optional<Pair<DataBox, Long>> res = putHelp(key, rid);
        //if this node's children split
        if(res.isPresent()){
            res = this.InnerPutHelp(res.get().getFirst(), res.get().getSecond());
        }
        return res;
        }
    ...
    private Optional<Pair<DataBox, Long>> putHelp(DataBox key, RecordId rid) {
        int thisOrder = metadata.getOrder();
        int flag = numLessThanEqual(key, keys);
        Page p = bufferManager.fetchPage(treeContext, children.get(flag));
        try {
            Buffer buf = p.getBuffer();
            byte b = buf.get();
            if (b == 1) {
                LeafNode thisLeafNode = LeafNode.fromBytes(metadata, bufferManager, treeContext, children.get(flag));
                return thisLeafNode.put(key, rid);
            } else if (b == 0) {
                InnerNode thisInnerNode = InnerNode.fromBytes(metadata, bufferManager, treeContext, children.get(flag));
                Optional<Pair<DataBox, Long>> res = thisInnerNode.putHelp(key, rid);
                if(res.isPresent()){
                    res = thisInnerNode.InnerPutHelp(res.get().getFirst(), res.get().getSecond());
                }
                return res;
            } else {
                String msg = String.format("Unexpected byte %b.", b);
    ...
    /*flag 1:leaf to InnerNode,0:InnerNode to InnerNode*/
    private Optional<Pair<DataBox, Long>> InnerPutHelp(DataBox key, Long child) {
        int thisOrder = this.metadata.getOrder();
        int flag1 = 0;
        for (DataBox dataBox : this.keys) {
            if (key.compareTo(dataBox) > 0) {
                flag1++;
            }
        }
        this.keys.add(flag1, key);
        //the children is keys.size() + 1
        this.children.add(flag1 + 1, child);

        if (keys.size() == thisOrder * 2 + 1) {//if split
            List<DataBox> newKeys = new ArrayList<>();
            List<Long> newChild = new ArrayList<>();
            //Insert key to the order sequence
            for (int i = thisOrder; i < thisOrder * 2 + 1; i++) {
                newKeys.add(this.keys.get(thisOrder));
                this.keys.remove(thisOrder);
            }
            for (int i = thisOrder + 1; i < thisOrder * 2 + 2; i++) {
                newChild.add(this.children.get(thisOrder + 1));
                this.children.remove(thisOrder + 1);
            }
            //if the case:InnerNode to InnerNode:need to move the first key of the new InnerNode to its parent
            Page newPage = bufferManager.fetchNewPage(treeContext, metadata.getPartNum());
            Pair<DataBox, Long> res = new Pair<>(newKeys.remove(0), newPage.getPageNum());
            InnerNode newNode = new InnerNode(metadata, bufferManager, newPage, newKeys, newChild, this.treeContext);
            this.sync();
            return Optional.of(res);//move to up InnerNode
        } else {
            this.sync();
